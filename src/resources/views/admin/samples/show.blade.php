    ...
    <div class="card mb-3">
        ...
        <div class="card-body">
            <div class="row">
                ...
                <div class="col-md-3 mb-2"><strong>Size:</strong> {{ $sample->size->name ?? $sample->size_ref ?? '-' }}</div>
                ...
                <div class="col-md-3 mb-2">
                    <strong>Submission Date:</strong> {{ optional($sample->submission_date)->format('d M Y') ?? '-' }}
                ...
                @if($sample->parentSample)
                    <div class="col-12 mb-2"><strong>Revised from:</strong> <a href="{{ route('merchandising-trace.samples.show', $sample->parentSample) }}">{{ $sample->parentSample->sample_number }}</a></div>
                @endif
            </div>
            ...
            @can('merch_sample.edit')
                ...
                <div class="d-flex flex-wrap gap-2">
                    @if($sample->status !== 'submitted' && $sample->status !== 'approved')
                    ...
                    @if(in_array($sample->status, ['submitted', 'in_progress']))
                        ...
                        <button type="button" class="btn btn-sm btn-danger" data-toggle="modal" data-target="#rejectModal"><i class="fa-solid fa-xmark"></i> Reject</button>
                    @endif
                    @if(in_array($sample->status, ['rejected', 'resubmit']) && $sample->revisions->isEmpty())
                        <form method="POST" action="{{ route('merchandising-trace.samples.resubmit', $sample) }}">
                            @csrf
                            <button type="submit" class="btn btn-sm btn-dark"><i class="fa-solid fa-rotate"></i> Create Next Revision</button>
                        </form>
                    @endif
                </div>
            @endcan
        ...
        <div class="card mb-3">
            ...
            <div class="card-body">
                ...
                @foreach($chain->sortBy('revision_no') as $rev)
                    <a href="{{ route('merchandising-trace.samples.show', $rev) }}" class="badge {{ $rev->id === $sample->id ? 'bg-primary' : 'bg-secondary' }} text-decoration-none me-1">
                        rev {{ $rev->revision_no }} — {{ ucfirst(str_replace('_', ' ', $rev->status)) }}
                    </a>
                @endforeach
            </div>
        </div>
            ...
            <div class="modal-content">
                <form method="POST" action="{{ route('merchandising-trace.samples.submit', $sample) }}">
                    ...
                    <div class="modal-body">
                        <div class="mb-3"><label class="form-label">Submission Date <span class="text-danger">*</span></label><input type="date" name="submission_date" class="form-control" value="{{ now()->toDateString() }}" required></div>
