        ...
        <div class="card-body">
            <form method="GET" class="row mb-3 align-items-end">
                <div class="col-md-3 mb-2">
                    <input type="text" name="search" class="form-control form-control-sm" placeholder="Search BOM no" value="{{ request('search') }}">
                </div>
                <div class="col-md-3 mb-2">
                ...
                <div class="col-md-3 mb-2 d-flex align-items-end flex-wrap gap-1">
                    ...
                    <a href="{{ route('merchandising-trace.boms.index') }}" class="btn btn-light btn-sm">Reset</a>
                </div>
            </form>
            ...
            <div class="table-responsive">
                <table class="table table-bordered table-sm align-middle">
                    <thead>
                        <tr><th>#</th><th>BOM No</th><th>Style</th><th>Version</th><th>Option</th><th>Status</th><th class="text-right">Actions</th></tr>
                    </thead>
                    <tbody>
                        @forelse($boms as $bom)
                            <tr>
                                ...
                                <td><span class="badge badge-secondary">{{ ucfirst($bom->status) }}</span></td>
                                <td class="text-right">
                                    <a href="{{ route('merchandising-trace.boms.show', $bom) }}" class="btn-custom success"><i class="fa-solid fa-eye"></i></a>
                                    @can('merch_bom.delete')
                                        <button type="button" class="btn-custom danger" data-toggle="modal"
                                            data-target="#deleteBomModal" data-action="{{ route('merchandising-trace.boms.destroy', $bom) }}"><i class="fa-solid fa-trash"></i></button>
                                    @endcan
                                </td>
                            </tr>
                        @empty
