<div class="row">
<div class="col-md-3 mb-3">
    <label class="form-label">Code</label>
    <input type="text" name="code" class="form-control form-control-sm" value="{{ old('code', $item->code ?? '') }}" required>
</div>
<div class="col-md-3 mb-3">
    <label class="form-label">Name</label>
    <input type="text" name="name" class="form-control form-control-sm" value="{{ old('name', $item->name ?? '') }}" required>
</div>
<div class="col-md-3 mb-3">
    <label class="form-label">Type</label>
    <select name="type" class="form-control form-control-sm" required>
        <option value="">— Select —</option>
        @foreach(\ME\MerchandisingTrace\Models\Item::TYPES as $t)
            <option value="{{ $t }}" @selected(old('type', $item->type ?? '') === $t)>{{ ucfirst($t) }}</option>
        @endforeach
    </select>
</div>
<div class="col-md-3 mb-3">
    <label class="form-label">Category</label>
    <select name="category_id" class="form-control form-control-sm merch-select2">
        <option value="">— Select —</option>
        @foreach($categoriesOptions as $c)
            <option value="{{ $c->id }}" @selected(old('category_id', $item->category_id ?? '') == $c->id)>{{ $c->name }}</option>
        @endforeach
    </select>
</div>
<div class="col-md-3 mb-3">
    <label class="form-label">UOM</label>
    <select name="uom_id" class="form-control form-control-sm merch-select2">
        <option value="">— Select —</option>
        @foreach($uomsOptions as $u)
            <option value="{{ $u->id }}" @selected(old('uom_id', $item->uom_id ?? '') == $u->id)>{{ $u->name }}</option>
        @endforeach
    </select>
</div>
<div class="col-md-3 mb-3">
    <label class="form-label">Default Supplier</label>
    <select name="default_supplier_id" class="form-control form-control-sm merch-select2">
        <option value="">— Select —</option>
        @foreach($suppliersOptions as $s)
            <option value="{{ $s->id }}" @selected(old('default_supplier_id', $item->default_supplier_id ?? '') == $s->id)>{{ $s->name }}</option>
        @endforeach
    </select>
</div>
<div class="col-md-3 mb-3">
    <label class="form-label">Default Price</label>
    <input type="number" step="0.0001" name="default_price" class="form-control form-control-sm" value="{{ old('default_price', $item->default_price ?? '') }}">
</div>
<div class="col-md-3 mb-3">
    <label class="form-label">Consumption UOM</label>
    <input type="text" name="consumption_uom" class="form-control form-control-sm" value="{{ old('consumption_uom', $item->consumption_uom ?? '') }}">
</div>
<div class="col-md-3 mb-3 d-flex align-items-center"><div class="custom-control custom-switch">
    <input type="hidden" name="is_active" value="0">
    <input type="checkbox" name="is_active" value="1" class="custom-control-input" id="itemActive{{ $item->id ?? 'new' }}" @checked(old('is_active', $item->is_active ?? true))>
    <label class="custom-control-label" for="itemActive{{ $item->id ?? 'new' }}">Active</label>
</div>
</div></div>
