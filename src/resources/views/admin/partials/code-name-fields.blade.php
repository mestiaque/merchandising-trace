...
<div class="col-md-3 mb-3 d-flex align-items-center"><div class="custom-control custom-switch">
    ...
    <input type="checkbox" name="is_active" value="1" class="custom-control-input" id="{{ $idPrefix }}Active{{ $record->id ?? 'new' }}" @checked(old('is_active', $record->is_active ?? true))>
    <label class="custom-control-label" for="{{ $idPrefix }}Active{{ $record->id ?? 'new' }}">Active</label>
</div>
</div></div>
