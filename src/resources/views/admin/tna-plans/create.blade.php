            ...
            <form method="POST" action="{{ route('merchandising-trace.tna-plans.store') }}">
                ...
                <div class="row">
                    ...
                    <div class="col-md-4 mb-3">
                        ...
                        <input type="text" name="po_no" class="form-control" value="{{ old('po_no') }}" required>
                    </div>
                    <div class="col-md-4 mb-3">
                        <label class="form-label">PO Due Date</label>
                        <input type="date" name="po_due_date" class="form-control" value="{{ old('po_due_date') }}">
                    </div>
                    <div class="col-md-3 mb-3">
                        <label class="form-label">PO Qty <span class="text-danger">*</span></label>
                        <input type="number" min="0" name="po_qty" id="poQtyInput" class="form-control" value="{{ old('po_qty', 0) }}" required>
                    </div>
                    <div class="col-md-3 mb-3">
                        <label class="form-label">Unit Price</label>
                        <input type="number" step="0.0001" min="0" name="unit_price" class="form-control" value="{{ old('unit_price') }}">
                    </div>
                    <div class="col-md-3 mb-3">
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <input type="date" name="pcd_date" class="form-control" value="{{ old('pcd_date') }}">
                    </div>
                    <div class="col-md-6 mb-3">
                        <label class="form-label">Shipment Date</label>
                        <input type="date" name="shipment_date" class="form-control" value="{{ old('shipment_date') }}">
                    </div>
                </div>
                ...
                <div class="row">
                    @foreach($sizesOptions as $size)
                        <div class="col-md-2 mb-2">
                            ...
                            <input type="number" min="0" name="sizes[{{ $loop->index }}][qty]" class="form-control size-qty-input" value="{{ old('sizes.' . $loop->index . '.qty', 0) }}">
                        </div>
                    @endforeach
                </div>

                <button type="submit" class="btn btn-primary mt-3">Create T&amp;A Plan</button>
                <a href="{{ route('merchandising-trace.tna-plans.index') }}" class="btn btn-light mt-3">Cancel</a>
            </form>
        </div>
    </div>
</div>
