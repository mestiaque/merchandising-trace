...
<div class="row">
    ...
    <div class="col-md-3 mb-3">
        ...
        <input type="text" name="po_no" class="form-control form-control-sm" value="{{ old('po_no', $salesContractPo->po_no ?? '') }}" required>
    </div>
    <div class="col-md-3 mb-3">
        <label class="form-label">PO Due Date</label>
        <input type="date" name="po_due_date" class="form-control form-control-sm" value="{{ old('po_due_date', optional($salesContractPo->po_due_date ?? null)->format('Y-m-d')) }}">
    </div>
    <div class="col-md-3 mb-3">
        <label class="form-label">PO Qty <span class="text-danger">*</span></label>
        <input type="number" min="0" name="po_qty" id="poQtyInput" class="form-control form-control-sm" value="{{ old('po_qty', $salesContractPo->po_qty ?? 0) }}" required {{ isset($salesContractPo) ? 'readonly' : '' }}>
    ...
    <div class="col-md-3 mb-3">
        ...
        <input type="number" step="0.0001" min="0" name="unit_price" class="form-control form-control-sm" value="{{ old('unit_price', $salesContractPo->unit_price ?? '') }}">
    </div>
    <div class="col-md-3 mb-3">
    ...
    <div class="col-md-3 mb-3">
        ...
        <input type="number" step="0.01" min="0" name="cost_smv" class="form-control form-control-sm" value="{{ old('cost_smv', $salesContractPo->cost_smv ?? '') }}">
    </div>
    <div class="col-md-3 mb-3">
        <label class="form-label">CM</label>
        <input type="number" step="0.0001" min="0" name="cm" class="form-control form-control-sm" value="{{ old('cm', $salesContractPo->cm ?? '') }}">
    </div>
    <div class="col-md-3 mb-3">
        <label class="form-label">FOB/FOC</label>
        <input type="number" step="0.0001" min="0" name="fob_foc" class="form-control form-control-sm" value="{{ old('fob_foc', $salesContractPo->fob_foc ?? '') }}">
    </div>
    <div class="col-md-3 mb-3">
        <label class="form-label">PCD {{ isset($salesContractPo) ? '(base — see revisions below)' : '' }}</label>
        <input type="date" name="pcd_date" class="form-control form-control-sm" value="{{ old('pcd_date', optional($salesContractPo->pcd_date ?? null)->format('Y-m-d')) }}" {{ isset($salesContractPo) ? 'readonly' : '' }}>
    </div>
    <div class="col-md-3 mb-3">
        <label class="form-label">Shipment Date {{ isset($salesContractPo) ? '(base — see revisions below)' : '' }}</label>
        <input type="date" name="shipment_date" class="form-control form-control-sm" value="{{ old('shipment_date', optional($salesContractPo->shipment_date ?? null)->format('Y-m-d')) }}" {{ isset($salesContractPo) ? 'readonly' : '' }}>
    </div>
</div>
...
<div class="row">
    ...
    @foreach($sizesOptions as $size)
        <div class="col-md-3 mb-2">
            ...
            <input type="number" min="0" name="sizes[{{ $loop->index }}][qty]" class="form-control form-control-sm size-qty-input"
    ...
    <div class="row">
        @foreach([['field' => 'po_qty', 'label' => 'PO Qty', 'type' => 'number', 'current' => $salesContractPo->effectiveQty()], ['field' => 'pcd', 'label' => 'PCD', 'type' => 'date', 'current' => optional($salesContractPo->effectivePcd())->format('Y-m-d')], ['field' => 'shipment', 'label' => 'Shipment', 'type' => 'date', 'current' => optional($salesContractPo->effectiveShipment())->format('Y-m-d')]] as $rev)
            <div class="col-md-4 mb-3 border rounded p-2">
                <div class="font-weight-bold">{{ $rev['label'] }} <span class="text-muted small">(current: {{ $rev['current'] ?? '-' }})</span></div>
                <input type="{{ $rev['type'] }}" class="form-control form-control-sm mt-1 mb-1" id="revise_{{ $rev['field'] }}_value">
                <input type="text" class="form-control form-control-sm mb-1" id="revise_{{ $rev['field'] }}_reason" placeholder="Reason (required)">
